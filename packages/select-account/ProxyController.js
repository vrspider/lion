// import { fakeExtendsEventTarget } from '@lion/overlays/utils/fake-extends-target.js';
import { GlobalOverlayController, LocalOverlayController, overlays } from '@lion/overlays';
...
export class ProxyController extends EventTarget {
  /**
  ...
  constructor(config) {
    ...
    /** @type {GlobalOverlayController|LocalOverlayController} */
  ...
  _isGlobal(config) { // eslint-disable-line
    return Boolean(config.viewportConfig);
  }

  // TODO: most of this logic can be removed when GlobalOverlayController and Local are merged
  _init(config) {
    if (this._isGlobal(config)) {
      if (!this.__globalOverlayController) {
        this.__globalOverlayController = overlays.add(new GlobalOverlayController(config));
      }
      this.__activeCtrl = this.__globalOverlayController;
      // this.contentNode.style.position = 'fixed';
    } else {
      if (!this.__localOverlayController) {
        this.__localOverlayController = overlays.add(new LocalOverlayController(config));
      }
